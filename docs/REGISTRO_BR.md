...
> Feito em 2026-05-27 (#95): apex `aerobi.com.br` + `www` viraram de `35.219.200.207` (Firebase App
> Hosting) para `187.127.6.20` (VPS). Runbook abaixo mantido como referência para futuras migrações
> de apex.

Virar o A do apex para a VPS exige cuidado: o Certbot só emite o cert com o domínio já resolvendo para
`187.127.6.20`, mas virar antes do container estar de pé derruba a produção (o nginx não tem
`default_server`).
...
1. **~1h antes (opcional):** editar o A de `aerobi.com.br` e `www`, baixar TTL `3600 → 60` (rollback rápido).
2. Com o vhost só-HTTP já criado (`vhost_emit_cert=false`) e o container de pé: trocar o **Dados** do A de
   `35.219.200.207 → 187.127.6.20` para o apex e o `www`. Publicar a zona.
3. Validar propagação (`dig +short aerobi.com.br @e.sec.dns.br` no autoritativo + resolvers públicos)
   antes de emitir o cert.

> ⚠️ Não remover os registros de e-mail/verificação Google da zona (`MX`, `TXT v=spf1`, `CNAME` de
> verificação) — o cutover mexe **só** nos dois `A`. Os `TXT fah-claim=...` do Firebase são inofensivos
> e podem ser limpos depois.

**Rollback:** reverter o A para `35.219.200.207`. Com TTL 60, volta em minutos (com 3600, até ~1h).
